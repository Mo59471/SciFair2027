...
MPFIT Master Cosmology Fitter
    ...
    Obtains 10,000 time lags randomly sampled from error bars and corrected via power law for each object
    Produces 10,000 luminosity distances sampled from flux errors and time lag errors for each object
    Fits cosmological parameters to data over 1000 boostrap + MC resampling iterations
    Utilizes MPFIT as a least squares minimizer with logarithmic residuals to fit cosmology
...
omgTog = False #True means omega is fitted

propTog = True #Toggle use of additional proportionality correction from R-L relationship comparison 

if propTog:
    #From false cosmology with H0 = 10, omg_0 = 0.27
    propW1 = 0.858500951779403 
    propW1_118 = 0.7871855553377065
    propW1_28 = 0.7171336421897777
    propW2 = 0.8443455546344151
    propW2_118 = 0.9173568011167653
    propW2_28 = 1.3620658764726608
else:
    propW1 = 1 
    propW1_118 = 1 
    propW1_28 = 1
...
for i in range(len(sTauW1)):
    if sTauW1[i][0] != -999:
        ...
        for j in range(10000):
            ...
            DL_tauW1[i].append(propW1 * sTauW1[i][j] * 10**(0.2*(mV-kCorr-25+10.60)))
            DL_tauW1_118[i].append(propW1_118 * sTauW1_118[i][j] * 10**(0.2*(mV-kCorr-25+10.60)))
            DL_tauW1_28[i].append(propW1_28 * sTauW1_28[i][j] * 10**(0.2*(mV-kCorr-25+10.60)))
...
for i in range(len(sTauW2)):
    if sTauW2[i][0] != -999:
        ...
        for j in range(10000):
            mV = -2.5 * math.log10(sampleLogNormal(math.log10(flux[i]),logL_err[i])/(3640*1e-23)) #Assume a normalization of 3640 Jy, converted to erg/s/cm^2/Hz by factor of 10^-23
            DL_tauW2[i].append(propW2 * sTauW2[i][j] * 10**(0.2*(mV-kCorr-25+10.60)))
            DL_tauW2_118[i].append(propW2_118 * sTauW2_118[i][j] * 10**(0.2*(mV-kCorr-25+10.60)))
            DL_tauW2_28[i].append(propW2_28 * sTauW2_28[i][j] * 10**(0.2*(mV-kCorr-25+10.60)))
