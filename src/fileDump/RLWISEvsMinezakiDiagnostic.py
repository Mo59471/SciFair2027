...
def main():
    ...
    for i in range(len(flux_MAG)):
        if flux_MAG[i] != -999:
            ...
            for j in range(n):
                if str(obsBand[i]) == "R":
                    logLV_MAG[len(logLV_MAG)-1].append(
                        math.log10(flux_MAG[i] * (6283 / (5500*(1+zCMB_mag[i]))) ** np.random.choice(alphDistrL) * 4 * math.pi * DL**2 / (1+zCMB_mag[i]))
                    )
                elif str(obsBand[i]) == "I":
                    logLV_MAG[len(logLV_MAG)-1].append(
                        math.log10(flux_MAG[i] * (7774 / (5500*(1+zCMB_mag[i]))) ** np.random.choice(alphDistrL) * 4 * math.pi * DL**2 / (1+zCMB_mag[i]))
                    )
                else:
                    logLV_MAG[len(logLV_MAG)-1].append(
    ...
    for i in range(len(flux_WISE)):
        if flux_WISE[i] != -999:
            ...
            for j in range(n):
                if zHel[i] < 0.7:
                    logLV_WISE[len(logLV_WISE)-1].append(
                        math.log10(flux_WISE[i] * (5100 / 5500) ** np.median(alphDistrL) * 4 * math.pi * DL**2 / (1+zCMB[i]))
                    )
                elif zHel[i] < 1.9:
                    logLV_WISE[len(logLV_WISE)-1].append(
                        math.log10(flux_WISE[i] * (3000 / 5500) ** np.median(alphDistrL) * 4 * math.pi * DL**2 / (1+zCMB[i]))
                    )
                else:
                    logLV_WISE[len(logLV_WISE)-1].append(
                        math.log10(flux_WISE[i] * (1350 / 2200) ** np.median(alphDistrS) * (2200/5500) ** np.median(alphDistrL) * 4 * math.pi * DL**2 / (1+zCMB[i]))
                    )
        else:
    ...
    for i in range(len(sTauW1)):
        if sTauW1[i][0] != -999:
            cleanLogTauW1.append(np.log10(sTauW1[i]))
            cleanLogTauW1_118.append(np.log10(sTauW1_118[i]))
            cleanLogTauW1_28.append(np.log10(sTauW1_28[i]))
        ...
        if sTauW2[i][0] != -999:
            cleanLogTauW2.append(np.log10(sTauW2[i]))
            cleanLogTauW2_118.append(np.log10(sTauW2_118[i]))
            cleanLogTauW2_28.append(np.log10(sTauW2_28[i]))
    ...
    cleanLogTauW1_WISE = [np.log10(i) for i in sTauW1_WISE if i[0] != -999]
    cleanLogTauW1_WISE_err = [np.std(np.log10(i)) for i in sTauW1_WISE if i[0] != -999]
    cleanLogTauW2_WISE = [np.log10(i) for i in sTauW2_WISE if i[0] != -999]
    cleanLogTauW2_WISE_err = [np.std(np.log10(i)) for i in sTauW2_WISE if i[0] != -999]
    ...
    MPFIT_interceptW1 =  []
    MPFIT_interceptW1_err =  []
    MPFIT_interceptW1_118 =  []
    MPFIT_interceptW1_118_err  =  []
    MPFIT_interceptW1_28 =  []
    MPFIT_interceptW1_28_err =  []
    MPFIT_interceptW1_WISE =  []
    MPFIT_interceptW1_WISE_err  =  []
    
    MPFIT_interceptW2 =  []
    MPFIT_interceptW2_err =  []
    MPFIT_interceptW2_118 =  []
    MPFIT_interceptW2_118_err =  []
    MPFIT_interceptW2_28 =  []
    MPFIT_interceptW2_28_err =  []
    MPFIT_interceptW2_WISE  =  []
    MPFIT_interceptW2_WISE_err =  []
    MPFIT_interceptK  =  []
    MPFIT_interceptK_err  =  []
    ...
    for i in range(n):
        ...
        fitLogTauW2_118_err = []
        fitLogTauW2_28_err = []
        fitLogTauW1_WISE_err = []
        fitLogTauW2_WISE_err = []
        fitLogTauK_err = []
        
        while len(fitLogTauW1) < len(cleanLogTauW1):
            index = np.random.choice(len(cleanLogTauW1))
            sIndex = np.random.choice(n)
            fitLV_W1.append(cleanLV_W1[index][sIndex])
            fitLogTauW1.append(cleanLogTauW1[index][sIndex])
            fitLogTauW1_118.append(cleanLogTauW1_118[index][sIndex])
            fitLogTauW1_28.append(cleanLogTauW1_28[index][sIndex])
            fitLogTauW1_WISE.append(cleanLogTauW1_WISE[index][sIndex])
            fitLogTauW1_err.append(cleanLogTauW1_err[index])
            fitLogTauW1_118_err.append(cleanLogTauW1_118_err[index])
            fitLogTauW1_28_err.append(cleanLogTauW1_28_err[index])
            fitLogTauW1_WISE_err.append(cleanLogTauW1_WISE_err[index])
            
        while len(fitLogTauW2) < len(cleanLogTauW2):
            index = np.random.choice(len(cleanLogTauW2))
            sIndex = np.random.choice(n)
            fitLV_W2.append(cleanLV_W2[index][sIndex])
            fitLogTauW2.append(cleanLogTauW2[index][sIndex])
            fitLogTauW2_118.append(cleanLogTauW2_118[index][sIndex])
            fitLogTauW2_28.append(cleanLogTauW2_28[index][sIndex])
            fitLogTauW2_WISE.append(cleanLogTauW2_WISE[index][sIndex])
            fitLogTauW2_err.append(cleanLogTauW2_err[index])
            fitLogTauW2_118_err.append(cleanLogTauW2_118_err[index])
            fitLogTauW2_28_err.append(cleanLogTauW2_28_err[index])
            fitLogTauW2_WISE_err.append(cleanLogTauW2_WISE_err[index])
        
        while len(fitLogTauK) < len(cleanLogTauK):
            index = np.random.choice(len(cleanLogTauK))
            sIndex = np.random.choice(n)
            fitLV_K.append(cleanLV_K[index][sIndex])
            fitLogTauK.append(cleanLogTauK[index])
            fitLogTauK_err.append(cleanLogTauK_err[index])
                    
        mW1 = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W1, dtype=float), "y":np.array(fitLogTauW1),"err":np.array(fitLogTauW1_err)},
            quiet=True
        )
        ...
        mW1_118 = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W1, dtype=float), "y":np.array(fitLogTauW1_118),"err":np.array(fitLogTauW1_118_err)},
            quiet=True
        )
        ...
        mW1_28 = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W1, dtype=float), "y":np.array(fitLogTauW1_28),"err":np.array(fitLogTauW1_28_err)},
            quiet=True
        )
        ...
        mW1_WISE = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W1, dtype=float), "y":np.array(fitLogTauW1_WISE),"err":np.array(fitLogTauW1_WISE_err)},
            quiet=True
        )
        ...
        mW2 = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W2, dtype=float), "y":np.array(fitLogTauW2),"err":np.array(fitLogTauW2_err)},
            quiet=True
        )
        ...
        mW2_118 = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W2, dtype=float), "y":np.array(fitLogTauW2_118),"err":np.array(fitLogTauW2_118_err)},
            quiet=True
        )
        ...
        mW2_28 = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W2, dtype=float), "y":np.array(fitLogTauW2_28),"err":np.array(fitLogTauW2_28_err)},
            quiet=True
        )
        ...
        mW2_WISE = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_W2, dtype=float), "y":np.array(fitLogTauW2_WISE),"err":np.array(fitLogTauW2_WISE_err)},
            quiet=True
        )
        ...
        mK = mpfit.mpfit(
            linefit,
            p0,
            functkw = {"x":np.array(fitLV_K, dtype=float), "y":np.array(fitLogTauK),"err":np.array(fitLogTauK_err)},
            quiet=True
        )
        
        MPFIT_interceptW1.append( float(mW1.params[0]))
        MPFIT_interceptW1_err.append(float(mW1.perror[0]))
        MPFIT_interceptW1_118.append( float(mW1_118.params[0]))
        MPFIT_interceptW1_118_err.append(float(mW1_118.perror[0]))
        MPFIT_interceptW1_28.append( float(mW1_28.params[0]))
        MPFIT_interceptW1_28_err.append(float(mW1_28.perror[0]))
        MPFIT_interceptW1_WISE.append(float(mW1_WISE.params[0]))
        MPFIT_interceptW1_WISE_err.append(float(mW1_WISE.perror[0]))
        
        MPFIT_interceptW2.append( float(mW2.params[0]))
        MPFIT_interceptW2_err.append(float(mW2.perror[0]))
        MPFIT_interceptW2_118.append( float(mW2_118.params[0]))
        MPFIT_interceptW2_118_err.append(float(mW2_118.perror[0]))
        MPFIT_interceptW2_28.append( float(mW2_28.params[0]))
        MPFIT_interceptW2_28_err.append(float(mW2_28.perror[0]))
        MPFIT_interceptW2_WISE.append(float(mW2_WISE.params[0]))
        MPFIT_interceptW2_WISE_err.append(float(mW2_WISE.perror[0]))

        MPFIT_interceptK.append(float(mK.params[0]))
        MPFIT_interceptK_err.append(float(mK.perror[0]))

    print("MPFIT best fit parameters:")
    print(f"MPFIT intercept W1, W2: {np.median(MPFIT_interceptW1)}, {np.median(MPFIT_interceptW2)}")
    print(f"MPFIT intercept W1_118, W2_118: {np.median(MPFIT_interceptW1_118)}, {np.median(MPFIT_interceptW2_118)}")
    print(f"MPFIT intercept W1_28, W2_28: {np.median(MPFIT_interceptW1_28)}, {np.median(MPFIT_interceptW2_28)}")
    print(f"MPFIT intercept W1_WISE, W2_WISE: {np.median(MPFIT_interceptW1_WISE)}, {np.median(MPFIT_interceptW2_WISE)}")
    print(f"MPFIT intercept K: {np.median(MPFIT_interceptK)}")
    print("")
    print("MPFIT errors:")
    print(f"MPFIT error W1, W2: {np.median(MPFIT_interceptW1_err)}, {np.median(MPFIT_interceptW2_err)}")
    print(f"MPFIT error W1_118, W2_118: {np.median(MPFIT_interceptW1_118_err)}, {np.median(MPFIT_interceptW2_118_err)}")
    print(f"MPFIT error W1_28, W2_28: {np.median(MPFIT_interceptW1_28_err)}, {np.median(MPFIT_interceptW2_28_err)}")
    print(f"MPFIT error W1_WISE, W2_WISE: {np.median(MPFIT_interceptW1_WISE_err)}, {np.median(MPFIT_interceptW2_WISE_err)}")
    print(f"MPFIT error K: {np.median(MPFIT_interceptK_err)}")
    print("")
    # print(f"Ratio K/W1: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1)+np.median(MPFIT_interceptW1_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1)-np.median(MPFIT_interceptW1_err))}")
    # print(f"Ratio K/W1_118: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_118))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1_118)+np.median(MPFIT_interceptW1_118_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_118))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_118))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1_118)-np.median(MPFIT_interceptW1_118_err))}")
    # print(f"Ratio K/W1_28: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_28))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1_28)+np.median(MPFIT_interceptW1_28_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_28))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_28))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1_28)-np.median(MPFIT_interceptW1_28_err))}")
    # print(f"Ratio K/W1_WISE: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_WISE))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1_WISE)+np.median(MPFIT_interceptW1_WISE_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_WISE))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_WISE))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW1_WISE)-np.median(MPFIT_interceptW1_WISE_err))}")
    # print(f"Ratio K/W2: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2)+np.median(MPFIT_interceptW2_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2)-np.median(MPFIT_interceptW2_err))}")
    # print(f"Ratio K/W2_118: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_118))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2_118)+np.median(MPFIT_interceptW2_118_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_118))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_118))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2_118)-np.median(MPFIT_interceptW2_118_err))}")
    # print(f"Ratio K/W2_28: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_28))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2_28)+np.median(MPFIT_interceptW2_28_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_28))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_28))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2_28)-np.median(MPFIT_interceptW2_28_err))}") 
    # print(f"Ratio K/W2_WISE: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_WISE))} +{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2_WISE)+np.median(MPFIT_interceptW2_WISE_err))-10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_WISE))}, -{10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_WISE))-10**(np.median(MPFIT_interceptK)+np.median(MPFIT_interceptK_err)-np.median(MPFIT_interceptW2_WISE)-np.median(MPFIT_interceptW2_WISE_err))}")

    MPFIT_interceptW1 = np.asarray(MPFIT_interceptW1)
    MPFIT_interceptW1_118 = np.asarray(MPFIT_interceptW1_118)
    MPFIT_interceptW1_28 = np.asarray(MPFIT_interceptW1_28)
    MPFIT_interceptW1_WISE = np.asarray(MPFIT_interceptW1_WISE)
    MPFIT_interceptW2 = np.asarray(MPFIT_interceptW2)
    MPFIT_interceptW2_118 = np.asarray(MPFIT_interceptW2_118)
    MPFIT_interceptW2_28 = np.asarray(MPFIT_interceptW2_28)
    MPFIT_interceptW2_WISE = np.asarray(MPFIT_interceptW2_WISE)
    MPFIT_interceptK = np.asarray(MPFIT_interceptK)

    

    print(f"Ratio K/W1: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW1))}")
    print(f"Ratio K/W1_118: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_118))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW1_118))}")
    print(f"Ratio K/W1_28: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_28))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW1_28))}")
    print(f"Ratio K/W1_WISE: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW1_WISE))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW1_WISE))}")
    print(f"Ratio K/W2: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW2))}")
    print(f"Ratio K/W2_118: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_118))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW2_118))}")
    print(f"Ratio K/W2_28: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_28))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW2_28))}")
    print(f"Ratio K/W2_WISE: {10**(np.median(MPFIT_interceptK)-np.median(MPFIT_interceptW2_WISE))} +-{np.std(10**(MPFIT_interceptK-MPFIT_interceptW2_WISE))}")
    print("")
